...
trait UserDaoT {
  ...
  def loginUser(email: String, password: String): Either[String, User] =  {
    DB.withConnection { implicit c =>
      val selectUser = SQL("Select id from Users where (email = {email}) and (password = {password}) and inactive=false").on(
      ...
      if (selectUser.isEmpty) {
        ...
        if (selectInactive.isEmpty) {
          Left("invalid")
        } else {
          Left("inactive")
        }
      } else {
        Right(returnActiveUser(selectUser.get))
      }
    }
  }
